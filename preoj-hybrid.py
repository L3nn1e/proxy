...
def parse_hysteria2_url(url: str) -> Dict[str, Any]:
    ...
    return {
        "protocol": "hysteria", # ✅ FIX: Xray-core uses "hysteria" for v2
        "settings": {
            "version": 2,       # ✅ FIX: Explicit version 2
            "address": host,
            "port": port,
            "auth": auth        # ✅ FIX: auth field
        },
        "streamSettings": {
            "network": "hysteria", # ✅ FIX: transport is "hysteria"
            "security": "tls",
            "tlsSettings": {
                "serverName": params.get('sni', [host])[0],
                "fingerprint": "chrome",
            ...
            "hysteriaSettings": { # ✅ FIX: settings block
                "version": 2,
                "up": "100 mbps",
                "down": "100 mbps"
            }
...
def parse_subscription_file(filepath: str) -> List[Dict[str, Any]]:
    ...
    for line in lines:
        ...
        try:
            ...
            # ✅ FILTER: Skip CN servers (check host and name)
            ...
            del config['host']
            del config['port']
            # keep 'proto' for internal use if needed, but delete for config generation usually
...
class LatencyChecker:
    ...
    async def _check(self, host: str, port: int) -> Tuple[bool, float]:
        start = time.time()
        try:
            # TCP connect check (reliable for all protocols including Hysteria v2 port)
    ...
    async def check_all(self, servers: List[Dict[str, Any]], concurrency: int = 30) -> List[Dict[str, Any]]:
        ...
        async def check(server):
            async with sem:
                # For Hysteria, we check the port. Xray needs the port to be open.
                # Settings location varies by protocol
                if 'settings' in server:
                    ...
                    if 'servers' in s:
                        ...
                        port = s['vnext'][0]['port']
                    elif 'address' in s: # Hysteria flat structure
                        host = s['address']
                        port = s['port']
                    else:
                        return server # skip check if unknown
                else:
                    return server
...
def generate_xray_config(servers: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for server in servers:
        # Filter dead servers if health check was run
    ...
    if tags:
        routing["balancers"] = [{
            "tag": "auto-balancer",
            "selector": tags,
            "strategy": {"type": "random"} # or "leastPing" if you prefer
        }]
...
async def main_async(args):
    ...
    # ✅ Apply clean_json to remove any whitespace in keys
